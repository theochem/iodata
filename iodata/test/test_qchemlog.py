...
def test_load_qchemlog_low_h2o():
    ...
    assert_allclose(
        data["polarizability_tensor"],
        [
            [-6.1256608, -0.1911917, 0.8593603],
...
def test_load_one_qchemlog_freq():
    ...
    assert_equal(
        mol.moments[(2, "c")], np.array([-6.1922, 0.2058, -0.9308, -5.0469, 1.1096, -5.762])
    )
    hessian = np.array(
        [
            [
    ...
    polarizability_tensor = np.array(
        [
            [-6.1256608, -0.1911917, 0.8593603],
            [-0.1911917, -7.180854, -1.0224452],
            [0.8593603, -1.0224452, -6.52088],
...
def test_load_qchemlog_low_qchemlog_h2o_dimer_eda2():
    ...
    atcoords = (
        np.array(
            [
                [-1.5510070000, -0.1145200000, 0.0000000000],
    ...
    assert_allclose(
        ...
        np.array([-0.610250, 0.304021, 0.337060, -0.663525, 0.316347, 0.316347]),
    )
...
def test_load_one_h2o_dimer_eda2():
    ...
    assert_equal(
        mol.atcharges["mulliken"],
        np.array([-0.610250, 0.304021, 0.337060, -0.663525, 0.316347, 0.316347]),
    )
    assert_allclose(mol.moments[(1, "c")], np.array([2.5689, 0.0770, 0.0000]))
    assert_allclose(
        mol.moments[(2, "c")], [-12.0581, -6.2544, -0.0000, -12.8954, -0.0000, -12.2310]
    )
