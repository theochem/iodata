...
def test_load_fchk_psi4_hcch():
    # Test FCHK file generated by Psi4, in which some floating point numbers are
    # not separated by whitespace, because their exponent has three digits.
    mol = load_fchk_helper("psi4_hcch.fchk")
    assert mol.title == "Generated by Psi4"
    assert mol.run_type == "energy"
    assert mol.lot == "rhf"
    assert mol.obasis_name == "def2-tzvp"
    assert_equal(mol.atnums, [1, 6, 6, 1])
    assert_equal(mol.atcorenums, [1.0, 6.0, 6.0, 1.0])
    # In the file, the x-components of the last two atoms are not separated
    # from the preceding number by whitespace.
    assert_allclose(
        mol.atcoords,
        [
            [-1.48851795e-117, 0.00000000e00, 3.13915557e00],
            [5.68635506e-118, 0.00000000e00, 1.13086932e00],
            [-3.46281773e-118, 0.00000000e00, -1.13086932e00],
            [-1.15900985e-117, 0.00000000e00, -3.13915557e00],
        ],
    )
    assert_allclose(mol.energy, -7.736452114876435e01)
    assert len(mol.obasis.shells) == 30
    assert mol.obasis.nbasis == 74
    assert_allclose(mol.obasis.shells[0].exponents, [34.061341, 5.1235746, 1.1646626])
    assert_equal(mol.mo.occs[:7], 2.0)
    assert_equal(mol.mo.occs[7:], 0.0)
    assert_allclose(mol.mo.energies[0], -1.01829056e01, atol=1.0e-7)
    assert_allclose(mol.mo.energies[6], -3.00511787e-01, atol=1.0e-7)
    assert_allclose(mol.mo.energies[7], 2.16228636e-02, atol=1.0e-7)
    assert_allclose(mol.mo.energies[-1], 2.27095744e01, atol=1.0e-7)
    assert_allclose(mol.mo.coeffs[0, 0], -6.13703434e-05, atol=1.0e-7)
    assert_allclose(mol.mo.coeffs[-1, -1], -1.98620260e-01, atol=1.0e-7)
    # Check molecular orbitals are orthonormal, which is only possible when the
    # atomic coordinates and the basis set are loaded correctly.
    olp = compute_overlap(mol.obasis, mol.atcoords)
    check_orthonormal(mol.mo.coeffs, olp)
    # The SCF density was stripped from the file to keep it below the size limit.
    assert mol.one_rdms == {}
