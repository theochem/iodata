...
def write_input(f: TextIO, data: IOData, template: str = None):
    ...
    # dictionary for converting run_type to Gaussian keyword
    ...
    fields = {"atnums": data.atnums,
              ...
              "run_type": data.run_type if not None else 'energy',
              # convert spin polarization to multiplicity
              "spinmult": int(data.spinpol) + 1,
              "charge": int(data.charge),
              "lot": data.lot if not None else 'hf',
              "obasis_name": data.obasis_name if not None else 'sto3g',
              }
