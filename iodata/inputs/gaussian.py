...
def write_input(f: TextIO, data: IOData, template: str = None):
    ...
    fields = {"atnums": data.atnums,
              ...
              "title": data.title if data.title is not None else 'Input Generated by IOData',
              "run_type": data.run_type if data.run_type is not None else 'energy',
              # convert spin polarization to multiplicity
              "spinmult": int(data.spinpol) + 1 if data.spinpol is not None else 1,
              "charge": int(data.charge) if data.charge is not None else 0,
