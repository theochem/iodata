...
def main():
    ...
    tfs = get_bare_transforms(args.lmax)
...
def parse_args():
    ...
    parser.add_argument("lmax", help="Maximum angular momentum", type=int)
...
def get_bare_transforms(lmax: int):
    ...
    for l in range(1, lmax + 1):
        ...
        for _m in range(2 * l + 1):
            ...
            for nx, ny, nz in iter_mononomials(l):
...
def real_regular_solid_harmonics(x, y, z, lmax, sqrt=None):
    """Evaluate the real regular solid harmonics up the l=lmax.
    ...
    -------
    result
        List of real regular solid harmonics up to angular momentum lmax.
    ...
    for l in range(2, lmax + 1):
        ...
        p2 = l - 1
        p3 = l
        c_l2_0 = result[offset2]  # C_{l-2, 0}
        c_l1_0 = result[offset1]  # C_{l-1, 0}
        result.append((p1 * z * c_l1_0 - p2 * r2 * c_l2_0) / p3)
        # case m = 1 ... l - 2
        for m in range(1, l - 1):
            p2 = sqrt((l + m - 1) * (l - m - 1))
            p3 = sqrt((l + m) * (l - m))
            c_l2_m = result[offset2 + 2 * m - 1]  # C_{l-2, m}
            c_l1_m = result[offset1 + 2 * m - 1]  # C_{l-1, m}
            result.append((p1 * z * c_l1_m - p2 * r2 * c_l2_m) / p3)
            s_l2_m = result[offset2 + 2 * m]  # S_{l-2, m}
            s_l1_m = result[offset1 + 2 * m]  # S_{l-1, m}
            result.append((p1 * z * s_l1_m - p2 * r2 * s_l2_m) / p3)
        # case m = l - 1
        p4 = sqrt(p1)
        c_l1_m = result[offset1 + 2 * l - 3]  # C_{l-1, l-1}
        result.append(p4 * z * c_l1_m)
        s_l1_m = result[offset1 + 2 * l - 2]  # S_{l-1, l-1}
...
def include_l2_norm(tfs):
    ...
    for l, tf in enumerate(tfs):
        ...
        # as it is consistent
        for i, (nx, ny, nz) in enumerate(iter_mononomials(l)):
...
def print_latex(tfs):
    ...
    for l, tf in enumerate(tfs):
        ...
        print("   ", r" \\ ".join(["b({})".format(label) for label in iter_pure_labels(l)]))
...
def test_library():
    ...
    for l in range(1, lmax + 1):
        for m in range(l + 1):
            # cosine line
            assert rrsh.pop(0).expand() == _get_cartfn(l, m), (l, m)
            if m > 0:
                # sine like
                assert rrsh.pop(0).expand() == _get_cartfn(l, -m), (l, m)
